// Section12_Challenge_MySolution.cpp : This file contains the 'main' function. Program execution begins and ends there.
...
int* apply_all(const int* const array1, const size_t array1_size, const int* const array2, const size_t array2_size) {
    int* productArray{ new int[array1_size * array2_size] };

    int pos{ 0 };
    for (size_t i {0}; i < array2_size; i++) {
        for (size_t j {0}; j < array1_size; j++) {
            productArray[pos] = array2[i] * array1[j];
            pos++;
        }
    }
    return productArray;
