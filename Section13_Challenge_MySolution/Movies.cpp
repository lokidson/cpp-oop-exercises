...
bool Movies::increment_watched(std::string name) {
    ...
    for (size_t i{ 0 }; i < movies.size(); i++) {
        if (movies[i].get_name() == name) {
            int currentWatched{ movies[i].get_watched() };
            movies[i].set_watched(++currentWatched);
            return true;
        }
    }
    return false;
...
/*************************************************************************
    ...
    Custom method by LK - Check 'movies' vector for the given movie name to see if
    a given title is already in the collection or not. Return true if found.
    *********************************************************************/
bool Movies::checkForTitle(std::string name) {
